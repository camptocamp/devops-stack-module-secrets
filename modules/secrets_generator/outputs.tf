...
output "secrets_names_without_secret_store" {
  ...
  value = {
    loki_stack = {
      logs_storage = local.secrets_to_create.logs_storage_secret != null ? local.secrets_to_create.logs_storage_secret.name : null
    }
    kube_prometheus_stack = {
      alertmanager_secrets       = local.secrets_to_create.alertmanager_secrets != null ? local.secrets_to_create.alertmanager_secrets.name : null
      grafana_admin_credentials  = local.secrets_to_create.grafana_admin_credentials.name
      metrics_storage            = local.secrets_to_create.metrics_storage_secret != null ? local.secrets_to_create.metrics_storage_secret.name : null
